...
class RunTests(unittest.TestCase):
  def _runCommand(self, command, logPath, expectedExitCode = 0):
    with open(logPath, 'w+') as out:
      #command = command.split(' ',1) #change
      #command = command[0] + ' wast ' + command[1] #change
      print(command)
  ...
  def _runTestFile(self, inputPath):
    ...
    newWasmCommand = "\"" + wasmCommand + "\"" + ' wast'
    print(wasmCommand)
    
    # Run original file
    expectedExitCode = 1 if ".fail." in inputFile else 0
    logPath = self._auxFile(outputPath + ".log")
    self._runCommand(('%s "%s"') % (newWasmCommand, inputPath), logPath, expectedExitCode)
...
"""
    ...
    self._runCommand(('%s -d "%s" -o "%s"') % (newWasmCommand, inputPath, wasmPath), logPath)
    self._runCommand(('%s "%s"') % (newWasmCommand, wasmPath), logPath)

    # Convert back to text and run again
    wastPath = self._auxFile(wasmPath + ".wast")
    logPath = self._auxFile(wastPath + ".log")
    self._runCommand(('%s -d "%s" -o "%s"') % (newWasmCommand, wasmPath, wastPath), logPath)
    self._runCommand(('%s "%s"') % (newWasmCommand, wastPath), logPath)

    # Convert back to binary once more and compare
    wasm2Path = self._auxFile(wastPath + ".bin.wast")
    logPath = self._auxFile(wasm2Path + ".log")
    self._runCommand(('%s -d "%s" -o "%s"') % (newWasmCommand, wastPath, wasm2Path), logPath)
    ...
    self._runCommand(('%s -d "%s" -o "%s"') % (newWasmCommand, wasm2Path, wast2Path), logPath)
    self._compareFile(wastPath, wast2Path)

    jsPath = self._auxFile(outputPath.replace(".wast", ".js"))
    logPath = self._auxFile(jsPath + ".log")
    self._runCommand(('%s -d "%s" -o "%s"') % (newWasmCommand, inputPath, jsPath), logPath)
    if jsCommand != None:
