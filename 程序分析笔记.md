...
- must analysis，因此控制流处理是取交集，而非取并集

**可用表达式的算法**
<div align=center> 
<img src="./assets/SPA/4.2-1.png" height="150"></img>   
</div>  

##### 1.2.3.5 Predictable Expression Analysis 可预测表达式分析
表达式e在程序块B的出口处被认为是可预测的，当且仅当：
- 每条离开B的代码路径都对e进行求值并后续使用它
- 且在B的末尾处对e进行求值，所得结果与沿任一路径回溯到e第一次求值的结果都是相同的
也就意味着e在B中的一次求值可用于预测其沿所有代码路径的任一后续求值结果。
可预测性分析的结果会用于代码移动中，一是为了减少执行时间，如用于缓式代码移动；二是减小编译后的代码长度，如用于代码提升

**可预测表达式分析算法**
<div align=center> 
<img src="./assets/SPA/4.2-2.jpg" height="150"></img>   
</div>  

##### 1.2.3.6 过程间综述问题
分析单个过程时，编译器必须考虑每个过程调用带来的影响。在缺少特定调用的具体信息的情况下，必须做好最坏的假定。考虑到被调用者的所有可能操作和其中再次调用其他被调用者的操作。这些最坏情况假定可能会严重降低全局数据流信息的质量。比如，编译器必须假定被调用者会修改其可以访问的每个变量，这个假定实质上停止了跨越调用位置传播所有全局变量、模块级变量和引用调用参数的相关事实。
为限制这样的影响，编译器可以在每个调用位置上计算综述信息。经典的综述问题会计算调用可能修改的变量的集合，以及可能被用作调用结果的度量集合。
过程间可能修改(interprocedural may modify)问题的目的在于：将被调用者及其调用过程可能修改的变量名的集合计算出来，标注到每个对应的调用位置上。**可能修改**是过程间分析领域最简单的问题之一。
...
## 3.DWARF
DWARF提供了程序运行时信息到源码信息的映射。弥补了ptrace无法获取源码信息的不足。
```
IP|regs|address      >>>  Source File 
(Runtime)                 DWARF    
```

[DWARF文档](https://dwarfstd.org/Dwarf5Std.php) 
...
对于GCC和Clang，使用参数`-gdwarf-4`就能生成DWARF4调试信息
DWARF文件是一个Mach-O类型文件：
```
foo: Mach-O 64-bit dSYM companion file x86_64
查看Mach-O类型的dwarf文件中的段，里面能找到__DWARF段，下面有__debug_line, __debug_pubnames, __debug_pubtypes等
...
采用dwarfdump可以探索DWARF内容。DWARF用The Debugging Information Entry (DIE) 来以统一的形式描述调试信息，每个DIE包含一个TAG属性(描述类型)，如：TAG_subprogram函数，和N个属性，用于具体描述一个DIE，如AT_name(函数名字)

```
# 此命令可展示__debug_line Section下的内容
...
0x0000000b: TAG_compile_unit [1] *
             ...
             AT_low_pc( 0x0000000100000f60 ) # 函数起始PC地址
             AT_high_pc( 0x0000004b ) # 函数结束PC地址
​
0x0000002a:     TAG_subprogram [2] *   # 函数
                 AT_low_pc( 0x0000000100000f60 )
                 AT_high_pc( 0x00000018 )
                 AT_frame_base( rbp )  # 函数的栈帧基址为rbp的值
                 AT_name( "foo" )
                 AT_decl_file( "foo.c" )
                 AT_decl_line( 1 )  # 函数再foo.c的第一行声明
                 AT_prototyped( true )  # bool值，为true表示这是一个子程序/函数
                 AT_type( {0x000000b2} ( int ) )  # 描述函数返回值类型
                 AT_external( true )  # 函数是否为全局可访问
​
0x00000043:         TAG_variable [3]  # 变量
                     AT_name( "d" )
...
​
0x00000058:         TAG_formal_parameter [4]  # 形式参数
                     AT_location( fbreg -4 )
                     AT_name( "a" )
                     AT_decl_file( "foo.c" )
                     AT_decl_line( 1 )
...
​
0x00000066:         TAG_formal_parameter [4]  # 形式参数
                     AT_location( fbreg -8 )
                     AT_name( "b" )
                     AT_decl_file( "foo.c" )
                     AT_decl_line( 1 )
...
0x00000082:         NULL
​
0x00000083:     TAG_base_type [6]  # 基础类型
                 AT_name( "double" )
                 AT_encoding( DW_ATE_float )
                 AT_byte_size( 0x08 )
​
0x0000008a:     TAG_subprogram [7] *  # 函数
...
0x000000b1:         NULL
​
0x000000b2:     TAG_base_type [6]  # 基础类型
                 AT_name( "int" )
...
#### 3.1.1 Mach-O文件
Mach-O为Mach Object文件格式缩写，一种用于可执行文件，目标代码，动态库，内核转储的文件格式。作为a.out格式的替代者，Mach-O提供了更强的扩展性。
Mach-O文件包括一个Mach-O头，然后是一系列的加载命令，再是一个或多个区块，每个块包括-到255个段。Mach-O使用REL再定位格式控制对符号的引用。Mach-O在两级名字空间中将每个符号编码成“对象-符号名”对，在查找符号时则采用线性搜索法。
